...
func is_point_occupied(position):
	for unit in get_unit_manager().get_all_units():
		if unit.character_id == character_id:
			continue
		if path_tiles.world_to_map(unit.position) == position:
			return true
	return false
...
func turn_actions():
	...
	if unit_priority:
		...
		var target_unit_node = get_unit_manager().get_targeted_unit(target_unit_world_position)
